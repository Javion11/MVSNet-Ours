...
class ConvBnReLU(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, pad=1):
        ...
        self.conv1_1 = nn.Conv2d(in_channels, out_channels, 1, stride=stride, padding=0, bias=False)
        self.conv = nn.Conv2d(in_channels, out_channels, kernel_size, stride=stride, padding=pad, bias=False)
        self.bn = nn.BatchNorm2d(out_channels)

    def forward(self, x):
        if self.in_channels != self.out_channels:
            res_tmp = self.bn(self.conv(x)) + self.bn(self.conv1_1(x))
        if self.in_channels == self.out_channels:
            res_tmp = self.bn(self.conv(x)) + self.bn(self.conv1_1(x)) + x
        return F.relu(res_tmp, inplace=True)
...
class ConvBnReLU3D(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, pad=1):
        ...
        self.in_channels = in_channels
        self.out_channels = out_channels
        self.conv1_1 = nn.Conv3d(in_channels, out_channels, 1, stride=stride, padding=0, bias=False)
