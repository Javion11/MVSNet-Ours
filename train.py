...
# model, optimizer
# NOTE: select network and whether refine module to be add 
...
def train_sample(sample, detailed_summary=False):
    ...
    # NOTE:select which loss to use
    if args.refine == False:
        loss = model_loss(depth_est, depth_gt, mask)
    else:
        depth_refine_est = outputs["refined_depth"]
        loss1 = model_loss(depth_est, depth_gt, mask)
        loss2 = model_loss(depth_refine_est, depth_gt, mask)
        loss = loss1 + loss2
...
def test_sample(sample, detailed_summary=True):
    ...
    if args.refine == False:
        ...
        depth_refined_est = outputs["refined_depth"]
        loss1 = model_loss(depth_est, depth_gt, mask)
        loss2 = model_loss(depth_refined_est, depth_gt, mask)
        loss = loss1 + loss2
